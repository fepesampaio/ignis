...
import { MathRichTextEditor } from '@/components/ui/math-rich-text-editor';
...
export function QuestionFormDialog({ open, onOpenChange, examId, question }: QuestionFormDialogProps) {
  const queryClient = useQueryClient();
  const [options, setOptions] = useState<Option[]>(getEmptyOptions());
  ...
  useEffect(() => {
    if (open) {
      if (question) {
        ...
        // Explicitly reset everything for new question
        form.reset({
          question_text: '',
          points: 1,
        });
              ...
              >
                {options.map((option, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <Label htmlFor={`option-${index}`} className="w-8 shrink-0">
